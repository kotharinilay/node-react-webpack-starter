...
import { deleteBreed as deleteBreedRecords } from '../../../../services/private/setup';
...
class BreedDisplay extends Component {
    constructor(props) {
        ...
        this.state = {
            columns: [
                ...
                { field: 'SpeciesId', isSort: false, displayName: 'Species Id' },
                { field: 'BreedTypeId', isSort: false, displayName: 'BreedType Id' },
                { field: 'Id', width: '35px', isSort: false, displayName: '', isExpand: true, formatter: this.expander.bind(this), visible: true },
                { field: 'SpeciesNameCode', displayName: 'Species', visible: true },
                { field: 'BreedTypeNameCode', displayName: 'Breed Type', visible: true, format: 'emptyFormat' }
            ],
            subColumns: [
                { field: 'Id', isKey: true, isSort: false, displayName: 'Breed Id' },
                { field: 'AuditLogId', isKey: false, isSort: false, displayName: 'AuditLog Id', visible: false },
                { field: 'BreedCode', displayName: 'Breed Code', visible: true },
                { field: 'BreedName', displayName: 'Breed Name', visible: true },
        ...
        this.deleteBreedClick = this.deleteBreedClick.bind(this);
        this.deleteBreed = this.deleteBreed.bind(this);
        this.modifyBreed = this.modifyBreed.bind(this);
    ...
    componentWillReceiveProps(nextProps) {
        ...
        this.refs.breedGrid.onSearchChange(nextProps.topSearch.searchText);
    }

    // Perform delete operation for selected breed
    deleteBreed() {
        ...
        let _this = this;
        deleteBreedRecords(uuids, auditLogIds).then(function (res) {
            if (res.success) {
                _this.props.notifyToaster(NOTIFY_SUCCESS, { message: _this.strings.DELETE_SUCCESS });
                _this.refs.breedGrid.refreshDatasource();
    ...
    // Open delete breed confirmation popup
    deleteBreedClick() {
        if (gridActionNotify(this.strings, this.props.notifyToaster, this.detailSelectedRows.length, true)) {
            ...
            let payload = {
                ...
                onConfirm: this.deleteBreed
            };
            this.props.openConfirmPopup(payload);
        }
    }

    // Redirect to edit mode for selected breed
    modifyBreed() {
        if (gridActionNotify(this.strings, this.props.notifyToaster, this.detailSelectedRows.length, true, true)) {
            browserHistory.push('/adminsetup/breed/' + this.detailSelectedRows[0].Id);
        }
    }

    // Clear grid selection
    clearSelection() {
        this.refs.breedGrid.cleanSelected();
    }
    ...
    expandComponent(row) {
        let gridProps = {
            maxHeight: "170px",
            functionName: 'breed/detaildataset',
            columns: this.state.subColumns,
            filterObj: { SpeciesId: row.SpeciesId, BreedTypeId: row.BreedTypeId },
    ...
    // Render breed components
    render() {
        let gridProps = {
            ...this.state,
            selectRowMode: 'none',
            functionName: 'breed/getdataset',
            expandBy: 'column',
            clickToExpand: true,
                            ...
                            <ul>
                                ...
                                <li><a href="javascript:void(0)" className="ripple-effect search-btn" data-toggle="dropdown">{this.strings.CONTROLS.ACTION_LABEL}</a>
                                    ...
                                    <ul className="dropdown-menu mega-dropdown-menu action-menu action-menu-height">
                                        <li>
                                            <ul>
                                                <li><a href="javascript:void(0)" onClick={() => browserHistory.replace('/adminsetup/breed/new')}> {this.strings.CONTROLS.NEW_LABEL}</a></li>
                                                <li><a href="javascript:void(0)" onClick={this.modifyBreed}> {this.strings.CONTROLS.MODIFY_LABEL}</a></li>
                                                <li><a href="javascript:void(0)" onClick={this.deleteBreedClick}> {this.strings.CONTROLS.DELETE_LABEL}</a></li>
                                            </ul>
                                        </li>
                                    </ul>
                                </li>
