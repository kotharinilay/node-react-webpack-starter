...
import { validateCarcass, importCarcass } from '../../../../../services/private/import-desk';
import { carcassColumns } from '../../../../../../shared/csv';

class ImportCarcass extends Component {
    constructor(props) {
        super(props);
        ...
        this.mappingColums = [
            ...
            { field: 'CSVColumns', displayName: 'CSV Columns', visible: true, editable: false },
            {
                ...
                editable: { type: 'select', options: { values: carcassColumns } }
            }
        ];
    ...
    validateClick() {
        ...
        let _this = this;
        return validateCarcass(mapping, this.firebaseKey, this.uploadedFileData, this.identifier, this.props.topPIC).then(function (res) {
            if (res.success) {
        ...
        if (this.step1) {
            return (<div className={this.state.currentStep == 1 ? '' : 'hidden'}>
                ...
                <DirectFileUpload label={this.strings.UPLOAD_CSV_LABEL} strings={this.strings.COMMON}
                    firebaseKey={this.firebaseKey} importType='import-carcass'
    ...
    renderStep2() {
        if (this.step2) {
            if (this.refs.fileData && this.refs.fileData.isNewFile) {
                ...
                let importTagMapColumns = carcassColumns.toString().replace(/ +/g, "").toLowerCase().split(',');
                columnData.forEach(function (element, i) {
                    ...
                    if (findIndex != -1)
                        this.mappingGridData.push({ index: i, CSVColumns: element, mapColumn: carcassColumns[findIndex] });
                    else
                ...
                <div className='stock-list'>
                    <div className="stock-list-cover">
                        ...
                        <div className="col-md-3 guide-main">
                            ...
                            <br />
                            <a style={{ color: '#006db7' }} href={this.siteURL + "/static/csv-templates/import-carcass.csv"} download>{this.strings.DOWNLOAD_CSV_LINK}</a>
                        </div>
                    </div>
                </div>
            </div>
