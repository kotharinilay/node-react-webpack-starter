...
import { tagColumns } from '../../../../../../shared/csv';

class ImportTag extends Component {
    constructor(props) {
        super(props);
        ...
        this.state = {
            ...
            resetStep1: Math.random(),
            resetStep2: Math.random(),
        };
        ...
        this.mappingColums = [
            ...
            { field: 'CSVColumns', displayName: 'CSV Columns', visible: true, editable: false },
            {
                ...
                editable: { type: 'select', options: { values: tagColumns } }
            }
        ];
    ...
    validateClick() {
        ...
        let _this = this;
        return validateTags(mapping, this.firebaseKey, this.uploadedFileData).then(function (res) {
            if (res.success) {
    ...
    renderStep1() {
        return (
            <div key={this.state.resetStep1} className={this.state.currentStep == 1 ? '' : 'hidden'}>
                <DirectFileUpload label={this.strings.UPLOAD_CSV_LABEL} strings={this.strings.COMMON}
                    firebaseKey={this.firebaseKey} importType='import-tag'
    ...
    renderStep2() {
        if (this.step2) {
            if (this.refs.fileData && this.refs.fileData.isNewFile) {
                ...
                let importTagMapColumns = tagColumns.toString().replace(/ +/g, "").toLowerCase().split(',');
                columnData.forEach(function (element, i) {
                    let tempEle = element.replace(/(?:\r)/g, '').replace(/ +/g, "").toLowerCase();
                    let findIndex = importTagMapColumns.indexOf(tempEle);
                    if (findIndex != -1)
                        this.mappingGridData.push({ index: i, CSVColumns: element, mapColumn: tagColumns[findIndex] });
                    else
            ...
            return (<div key={this.state.resetStep2} className={this.state.currentStep == 2 ? '' : 'hidden'}>
                ...
                <div className='stock-list'>
                    <div className="stock-list-cover">
                        ...
                        <div className="col-md-3 guide-main">
                            ...
                            <br />
                            <a style={{ color: '#006db7' }} href={this.siteURL + "/static/csv-templates/import-tag.csv"} download>{this.strings.DOWNLOAD_CSV_LINK}</a>
                        </div>
                    </div>
                </div>
            </div>
