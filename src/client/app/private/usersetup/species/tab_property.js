...
/**************************
 * Property tab to display species
 * **************************** */
...
class PropertyTab extends Component {
    ...
    rowSelect(selectedRow, row, isSelected) {
        let id = row.PropertyId ? row.PropertyId : row[0].PropertyId;
        this.props.updateResult(selectedRow, row, isSelected, id, false, false, false, true);
    }
    ...
    expandComponent(row) {
        ...
        let selectedRows = [];
        let selectedRowsObj = [];
        let resultData = this.props.data.resultData.property;
        row.expandData.map(s => {
            let rowObj = resultData.find(x => x.Property == row.expandData[0].PropertyId && x.Id.length > 0 && x.Id.includes(s.Id));
            if (!rowObj) {
    ...
    render() {
        let gridProps = {
            ...
            isRemoteData: false,
            gridData: this.props.data.property,
            clickToExpand: true,
