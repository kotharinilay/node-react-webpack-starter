// check polygon intersection : https://github.com/vrd/js-intersect
...
function inside(newOverlay, allOverlays) {
    //ensure the polygon contains enought vertices 
    if (!newOverlay)
        return true;
    else if (newOverlay && newOverlay.getPath().getLength() < 3)
        return true;

    var geometryFactory = new jsts.geom.GeometryFactory();
    var newPolygon = createJstsPolygon(geometryFactory, newOverlay);

    let insideAllPolygons = true;
    ...
    var result = allOverlays.filter(function (currentOverlay) {
        ...
        try {
            var inside = newPolygon.contains(curPolygon);
            if (!inside)
                insideAllPolygons = false;
        }
        catch (ex) {
            insideAllPolygons = false;
        }
...
function createJstsPolygon(geometryFactory, overlay) {
    ...
    // if (!shell.isSimple()) {
    //     return geometryFactory.createPolygon(shell).buffer(0);
    // }
...
function getEdges(fig) {
    ...
    var len = fig.length;
    for (var i = 0; i < len; i++) {
        edges.push([{ x: fig[(i % len)].x, y: fig[(i % len)].y }, {
            x: fig[((i + 1) % len)].x,
            y: fig[((i + 1) % len)].y
        }]);
    }
...
module.exports = {
    polygonIntersect: intersect,
    polygonInside: inside
}