...
exports.handler = function (event, context) {
    ...
    var path = require('path'),
        ...
        fileToProcess = event.body.fileToProcess, eidIndex = 1, rowIndex = 0;
    ...
    promise.map(s3GetPromises, function (c, index) {
        c.Body.toString().replace(/(?:\r)/g, '').split('\n').forEach(function (row) {
            ...
            let match = finalCSV.every((el, i) => {
                if (el[eidIndex] == element[eidIndex]) {
                    invalidGridData.push({ line: element[rowIndex], issues: `${element[eidIndex]} is duplicate.` });
                    invalidGridData.push({ line: el[rowIndex], issues: `${element[eidIndex]} is duplicate.` });
                    spliceIndex = i;
                    return false;
                }
                else
        ...
        context.done({ status: 500, message: err });
    });
}