...
exports.handler = function (event, context) {
    ...
    var fileToProcess = event.body.fileToProcess, picIndex = 4, rowIndex = 0;
    ...
    promise.map(s3GetPromises, function (c, index) {
        c.Body.toString().replace(/(?:\r)/g, '').split('\n').forEach(function (row) {
            ...
            let match = finalCSV.every((el, i) => {
                if (el[picIndex] == element[picIndex]) {
                    invalidGridData.push({ line: element[rowIndex], issues: `${element[picIndex]} is duplicate.` });
                    invalidGridData.push({ line: el[rowIndex], issues: `${element[picIndex]} is duplicate.` });
                    spliceIndex = i;
                    return false;
                }
                else
